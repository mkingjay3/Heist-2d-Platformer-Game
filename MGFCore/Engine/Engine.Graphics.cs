...
static partial class Engine
{
    ...
    private static readonly Dictionary<Tuple<Font, string>, TextCacheEntry> TextCache = new();
    private static readonly int TextCacheAgeLimit = 2;
    ...
    public static void DrawTexture(Texture texture, Vector2 position, Color? color = null, Vector2? size = null, float rotation = 0, Vector2? pivot = null, TextureMirror mirror = TextureMirror.None, Bounds2? source = null, TextureBlendMode blendMode = TextureBlendMode.Normal, TextureScaleMode scaleMode = TextureScaleMode.Linear)
    {
        ...
        DrawTexture(texture, new Bounds2(position, size ?? new Vector2(texture.Width, texture.Height)), color, rotation, pivot, mirror, source, blendMode, scaleMode);
    }

    public static void DrawTextureBatch(Texture texture, IEnumerable<Bounds2> destBoundsList, Color? color = null, float rotation = 0, Vector2? pivot = null, TextureMirror mirror = TextureMirror.None, Bounds2? source = null, TextureBlendMode blendMode = TextureBlendMode.Normal, TextureScaleMode scaleMode = TextureScaleMode.Linear)
    {
        DrawTextureSetup(texture.Handle, color, blendMode, scaleMode);
        foreach (Bounds2 destBounds in destBoundsList)
        {
            DrawTexture(texture, destBounds, color, rotation, pivot, mirror, source, blendMode, scaleMode);
        }
    }

    private static void DrawTexture(Texture texture, Bounds2 bounds, Color? color = null, float rotation = 0, Vector2? pivot = null, TextureMirror mirror = TextureMirror.None, Bounds2? source = null, TextureBlendMode blendMode = TextureBlendMode.Normal, TextureScaleMode scaleMode = TextureScaleMode.Linear)
    {
        ...
        if (source.HasValue)
        {
            ...
            dest.x = (int)bounds.Position.X;
            dest.y = (int)bounds.Position.Y;
            dest.w = src.w;
            dest.h = src.h;
        }
        else
        {
            ...
            dest.x = (int)bounds.Position.X;
            dest.y = (int)bounds.Position.Y;
            dest.w = (int)bounds.Size.X;
            dest.h = (int)bounds.Size.Y;
        }
    ...
    private static void FreeUnusedTextCacheEntries()
    {
        ...
        foreach (var keyAndEntry in TextCache)
        {
            ...
            if (keyAndEntry.Value.Age > TextCacheAgeLimit)
            {
