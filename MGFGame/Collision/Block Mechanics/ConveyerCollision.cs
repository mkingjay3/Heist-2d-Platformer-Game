...
{
    public class ConveyerCollision : ICollidable
    {
        private Tile tile;
        private float conveyerSpeed = 5f; 
        private bool movesRight = true; //to the right

        public ConveyerCollision(Tile tile, bool movesRight = true)
        {
            this.tile = tile;
        ...
        public void touch(Entity entity, Bounds2 entityBounds, Bounds2 tileBounds)
        {
            ...
            if (min == top)
            {
                // Player is on top of the conveyer
                ...
                // Only apply conveyer effect once per frame
                if (entity is Player player && !player.IsOnConveyer)
                {
                    player.IsOnConveyer = true;
                    float pushDirection = movesRight ? conveyerSpeed : -conveyerSpeed;
                    entity.Position = new Vector2(entity.Position.X + pushDirection, entity.Position.Y);
                }
            }
